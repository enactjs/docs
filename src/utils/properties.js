...
const processTypeTag = (tags) => {
	...
	// exist, add them with quotes around the value.
	// TODO: Add NumberLiteralType?
	const expression = `$[title="type"].type.[(
		$IsUnion := type = "UnionType";
		$quote := function($val) { "'" & $val & "'" };
		$GetNameExp := function($type) { $append($append($append($type[type="NameExpression"].name, $type[type="NullLiteral"] ? ['null'] : []), $type[type="AllLiteral"] ? ['Any'] : []), $map($type[type="StringLiteralType"].value, $quote)) };
