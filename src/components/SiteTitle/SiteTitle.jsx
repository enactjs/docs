...
const SiteTitle = ({children, custom, title, ...rest}) => {
	const {data, delimiter = ' | '} = rest;
	const siteTitle = 'Enact';

	const getSiteTitle = () => {
		// A fully customized title
		if (custom && title) return title;

		// Direct title
		if (title) return (title + delimiter + siteTitle);

		// Indirect title
		if (data && data.markdownRemark && data.markdownRemark.frontmatter && data.markdownRemark.frontmatter.title) {
			return (data.markdownRemark.frontmatter.title + delimiter + siteTitle);
		}
