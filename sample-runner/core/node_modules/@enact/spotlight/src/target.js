...
function getTargetInContainerByDirectionFromPosition(direction, containerId, positionRect, elementContainerIds, boundingRect) {
  ...
  var _loop = function _loop() {
      ...
      var nextCandidateContainerId = (0, _container.getContainersForNode)(next).pop();
      ...
      if (next && containerId !== nextCandidateContainerId) {
        ...
        if (isRestrictedContainer(nextCandidateContainerId) || isElementVisibleInContainer(next, nextCandidateContainerId)) {
          ...
          next = (0, _navigate["default"])(positionRect, direction, elementRects.filter(function (rect) {
            var elementContainerId = (0, _container.getContainersForNode)(rect.element).pop();
            return isElementVisibleInContainer(rect.element, elementContainerId);
          }), (0, _container.getContainerConfig)(containerId));
        }
      }
  ...
  var _loop2 = function _loop2() {
      ...
      if (next && containerId !== nextCandidateContainerId) {
        ...
        if (isRestrictedContainer(nextCandidateContainerId) || isElementVisibleInContainer(next, nextCandidateContainerId)) {
          ...
          // otherwise, try to navigate to an element that is visible in its container
          next = (0, _navigate["default"])(elementRect, direction, elementRects.filter(function (rect) {
            var elementContainerId = (0, _container.getContainersForNode)(rect.element).pop();
            return isElementVisibleInContainer(rect.element, elementContainerId);
          }), (0, _container.getContainerConfig)(containerId), partitionRect);
        }
      }
...
function isElementVisibleInContainer(element, containerId) {
  var _getContainerRect = (0, _utils.getContainerRect)(containerId),
    containerTop = _getContainerRect.top,
    containerRight = _getContainerRect.right,
    containerBottom = _getContainerRect.bottom,
    containerLeft = _getContainerRect.left;
  var _element$getBoundingC = element.getBoundingClientRect(),
    elementTop = _element$getBoundingC.top,
    elementRight = _element$getBoundingC.right,
    elementBottom = _element$getBoundingC.bottom,
    elementLeft = _element$getBoundingC.left;
  var isVerticallyVisible = elementBottom >= containerTop && elementTop <= containerBottom;
  var isHorizontallyVisible = elementRight >= containerLeft && elementLeft <= containerRight;
